...
1. rawData.csv
The raw questionnaire data, downloaded from the platform on which the experiment was conducted (soscisurvey.de).

2. data.csv
A table containing only data constituting the independent (body height + weight, age, musical + dance sophistication) and dependent (perceived tempo per stimulus) variables in wide format (one row per participant) and with deleted participants (n=4, unrealistic height or weight values).

3. script_git.R
The code in R (Version 20.24.12.1+563) used for analysis, containing the formatting into long format, several steps to prepocess the data, the statistical analyses using a linear mixed model (including assumption checks) and visualization.
